...
def create_mission_config(finish_action_value, global_height):
    ...
    exit_on_rc_lost.text = str(getattr(RCLostOptions.CONTINUE, "value", RCLostOptions.CONTINUE))

    execute_rc_lost_action = ET.SubElement(mission_config, "wpml:executeRCLostAction")
    execute_rc_lost_action.text = str(getattr(RCLostAction.GO_BACK, "value", RCLostAction.GO_BACK))
